...
/** Kimi/LiteLLM/etc.: env key + non-1P BASE_URL — no 1P verify round-trip. */
function hasThirdPartyEnvApiKey(): boolean {
  const apiKeyEnv = isRunningOnHomespace()
    ? undefined
    : process.env.ANTHROPIC_API_KEY
  return Boolean(apiKeyEnv && !isFirstPartyAnthropicBaseUrl())
}
